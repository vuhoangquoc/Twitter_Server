      ...
      email: {
        ...
        custom: {
          options: async (value, { req }) => {
            ...
            req.user = user // truyền qua usersController
            return true
          }
        }
      },
      ...
      refresh_token: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            try {
              const [decoded_refresh_token, find_refresh_token] = await Promise.all([
              ...
              // nếu dính lỗi của jsonwebtokenerror thì vào đây
              if (error instanceof JsonWebTokenError) {
                throw new ErrorWithStatus({
                  message: capitalize(error.message), // lấy trực tiếp của error
                  status: HTTP_STATUS.UNAUTHORIZED
                })
              }
              // còn không thì vào đây, nhận error ở trên kia
              throw error
            }
            return true
          }
...
export const forgotPasswordValidtor = validate(
  checkSchema(
    {
      email: {
        notEmpty: {
...
// nếu async thì phải next(new Error...)
  ...
  checkSchema(
    {
        ...
        custom: {
          options: async (value: string, { req }) => {
            if (!REGEX_USERNAME.test(value)) {
              // .test trả về true hoặc false
              throw Error(USERS_MESSAGES.USERNAME_INVALID)
            }
            const user = await databaseService.users.findOne({
              username: value
            })
            // Nếu đã tồn tại thì không cho phép cập nhật
            if (user) {
      ...
      cover_photo: {
        ...
        isLength: {
          ...
          errorMessage: USERS_MESSAGES.COVER_PHOTO_LENGTH
        }
      }
    },
    ['body']
